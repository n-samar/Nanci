...
p<-ggplot(p, aes(x=as.numeric(paste(size)), y=as.numeric(paste(latency)), group=size)) +
	     ...
	     ggtitle(paste("Parallel Binary Search on Mac (", toString(8), "B/elem)", sep="")) +
	     theme(plot.title = element_text(hjust = 0.5)) +
	     ylim(0, 0.3) +
	     theme(axis.text=element_text(size=12))	     

ggsave("../plots/parallel_bin_search.pdf", p, width=5, height=3, units="in", scale=1.5)
